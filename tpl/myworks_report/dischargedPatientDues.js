app.controller('dischargedPatientDuesController', ['$rootScope', '$scope', '$timeout', '$http', '$state', '$anchorScroll', '$filter', function ($rootScope, $scope, $timeout, $http, $state, $anchorScroll, $filter) {
        ...
        $scope.loadReport = function () {
            ...
            $http.post($rootScope.IRISOrgServiceUrl + '/myworkreports/dischargedpatientdues', data)
                    .success(function (response) {
                        $scope.records = $filter('filter')(response, function (value) { 
                            return value.viewChargeCalculation.balance > '0'
                        });
                        //$scope.records = $filter('filter')(response, {viewChargeCalculation: {balance: '> 0'}});
                        $scope.loadbar('hide');
                        $scope.loading = false;
